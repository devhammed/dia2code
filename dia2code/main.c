...
under certain conditions; read the COPYING file for details.\n";
  ...
  char *help = "[-h|--help] [-d <dir>] [-nc] [-cl <classlist>]\n\
       [-t (ada|c|cpp|csharp|idl|java|php|php5|laravel|ts|python|ruby|shp|sql)] [-v]\n\
  ...
  generators[12] = generate_code_ts;
  generators[13] = generate_code_laravel;
  ...
  for (i = 1; i < argc; i++)
  {
    ...
    case 1: /* Which code generator */
      ...
      else if (eq(argv[i], "ts"))
      {
        generator = generators[12];
      }
      else if (eq(argv[i], "laravel"))
      {
        generator = generators[13];
      }
      else
      {
#ifdef DSO
